...
class CBIMedSAM(nn.Module):
    ...
    def save_lora_parameters(self) -> dict:
        ...
        for i, w_A in enumerate(self.w_As):
            lora_state_dict[f"w_a_{i:03d}"] = w_A.weight.detach().cpu()
        for i, w_B in enumerate(self.w_Bs):
            lora_state_dict[f"w_b_{i:03d}"] = w_B.weight.detach().cpu()
        for i, w_G in enumerate(self.w_Gs):
            lora_state_dict[f"w_g_{i:03d}"] = w_G.weight.detach().cpu()

        # Save selected parts of SAM
        if isinstance(self.sam, (torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel)):
            sam_state_dict = self.sam.module.state_dict()
        else:
            sam_state_dict = self.sam.state_dict()

        sam_selected = {}
        for key, value in sam_state_dict.items():
            if any(substring in key for substring in ["mask_decoder", "freq", "prompt_encoder"]):
                sam_selected[key] = value.detach().cpu()

        # Merge
        merged_dict = {**lora_state_dict, **sam_selected}

        print(f"[Info] Saved {len(merged_dict)} parameters: "
              f"{len(lora_state_dict)} from LoRA, {len(sam_selected)} from SAM selective modules.")
    ...
    def load_lora_parameters(self, state_dict) -> None:
        ...
        for i, w_A_linear in enumerate(self.w_As):
            ...
            if saved_key in state_dict:
                ...
                print(f"[Warning] Missing {saved_key} in checkpoint, skipping.")
        ...
        for i, w_G_linear in enumerate(self.w_Gs):
            ...
            if saved_key in state_dict:
                w_G_linear.weight = nn.Parameter(state_dict[saved_key])
            else:
                print(f"[Warning] Missing {saved_key} in checkpoint, skipping.")
        ...
        for k in sam_dict.keys():
            if k in state_dict:
                ...
                # 打印提示但不会报错
                print(f"[Warning] {k} not found in checkpoint, using default initialized value.")
