...
from segment_anything.modeling.BoundaryRefineBlock import BoundaryRefineBlock
from segment_anything.modeling.CrissCrossAttention import CrissCrossAttention
from segment_anything.modeling.SEAttention import SEAttention
from segment_anything.modeling.LocalContextBlock import  LocalContextBlock
...
class SegField(nn.Module):
    def __init__(self,
        ...
        self.criss_cross = CrissCrossAttention(in_channels=features)

        self.local_context = LocalContextBlock(features)
    ...
    def forward(self, image_embedding, image_pe, original_shape, epoch_T=1.0, cls=0):
        ...
        seg_feat_concat = []
        for _ in range(2):
            seg_feat_ = self.seg_net[0](feat_concat)  # MLP coarse
            seg_feat_ = self.seg_net[2][cls](seg_feat_)  # 输出 (BHW, features + 1)

            # 提取 coarse mask + coarse features
            seg_coarse_temp = seg_feat_[:, :self.out_channel]  # mask logits
            seg_feat_temp = seg_feat_[:, self.out_channel:]  # (BHW, C)

            # reshape 为 (B, C, H, W) 送入 CCA
            seg_feat_temp = seg_feat_temp.view(b, h, w, -1).permute(0, 3, 1, 2).contiguous()
            seg_feat_temp = self.criss_cross(seg_feat_temp)  # CCA 增强
            seg_feat_temp = self.local_context(seg_feat_temp)
            seg_feat_temp = seg_feat_temp.permute(0, 2, 3, 1).contiguous().view(-1, seg_feat_temp.shape[1])  # 再拉平

            # 拼接 coarse mask 和增强后的特征
            seg_feat_enhanced = torch.cat([seg_coarse_temp, seg_feat_temp], dim=1)
            seg_feat_enhanced = self.drop_out(seg_feat_enhanced)

            seg_feat_concat.append(seg_feat_enhanced)
        ...
        if not hasattr(self, 'boundary_refine'):
            self.boundary_refine = BoundaryRefineBlock(in_channels=self.out_channel).to(seg_fine.device)

        seg_fine = self.boundary_refine(seg_fine)
        seg_res[-1] = seg_fine  # 替换 refine mask
...
if __name__ == "__main__":
    model = MaskDecoder(512)# 实例化模型，假设输入通道数为512
